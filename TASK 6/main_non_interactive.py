...
def visualize_data(df):
    ...
    plt.tight_layout()
    plt.savefig('iris_exploratory_analysis.png', dpi=300, bbox_inches='tight')
    print("✓ Exploratory data analysis plot saved as 'iris_exploratory_analysis.png'")
    plt.close()
...
def find_optimal_k(X_train_scaled, y_train):
    ...
    print("✓ Optimal k selection plot saved as 'optimal_k_selection.png'")
    plt.close()
...
def visualize_decision_boundary(X_train_scaled, y_train, knn):
    ...
    Z = knn_pca.predict(mesh_points)
    
    # Convert string labels to numeric for contour plotting
    label_encoder = {label: i for i, label in enumerate(knn_pca.classes_)}
    Z_numeric = np.array([label_encoder[label] for label in Z])
...
def create_summary_report(df, optimal_k, final_accuracy, best_params):
...
Key Findings:
- Optimal k (basic): {optimal_k}
- Best hyperparameters: {best_params}
- Final model accuracy: {final_accuracy:.4f} ({final_accuracy*100:.1f}%)
...
"""
    ...
    with open('analysis_report.txt', 'w') as f:
        f.write(report)
    print("✓ Detailed analysis report saved as 'analysis_report.txt'")
...
def main():
    ...
    create_summary_report(df, optimal_k, final_accuracy, best_knn.get_params())
    
    print("\n" + "=" * 60)
    print("KNN CLASSIFICATION ANALYSIS COMPLETE!")
    print("All plots and reports have been saved to files.")
    print("=" * 60)
